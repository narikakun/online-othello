    ...
    _ws.onopen = function() {
        ...
        _ws.onmessage = function(event) {
            ...
            if (data.leftHub) {
                if (WebSocketSettings.toGameRoomKey) {
                    let leftUser = null;
                    for (let i = 1; i <= WebSocketSettings.playerMax; i++) {
                        console.log(playerList[i]);
                        console.log(data);
                        if (!playerList[i]) continue;
                        if (playerList[i].id === data.user){
                            leftUser = i;
                            break;
                        }
                    }
                    ...
                    if (WebSocketSettings.host) {
                        ...
                        if (WebSocketSettings.roomHost === data.user) {
                            ...
                            WebSocketSettings.nickNameListA = {};
                            WebSocketSettings.roomHost = null;
                            WebSocketSettings.playerListRoom = [];
                            WebSocketSettings.closed = true;
                            _ws.close();
                            _ws = null;
...
function finishDataGoGoPanic() {
    ...
    let date = new Date();
    xhr.open('GET', 'https://gameapi.nakn.jp/othello/analytics/?resultInsert=1'+`&playerCount=${WebSocketSettings.playerListRoom.length}&gameId=${WebSocketSettings.roomKey}&endTime=${date.getFullYear()}-${1 + date.getMonth()}-${date.getDate()} ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}&roomKey=${WebSocketSettings.mRoomKey}`, true);
    xhr.send();
}
